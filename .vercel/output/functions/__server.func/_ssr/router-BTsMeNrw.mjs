...
//#region node_modules/.nitro/vite/services/ssr/assets/router-BTsMeNrw.js
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "max-w-md text-lg break-words text-ink-soft",
...
var SAMPLE_BOOK_ID = "sample-book";
var SAMPLE_CHAPTER_ID = "sample-ch-1";
var SAMPLE_BODY = `I never set out to write a book. I set out to remember.
...
function makeSampleBook(now = Date.now()) {
	return {
		book: {
			id: SAMPLE_BOOK_ID,
			...
			kind: "memoir",
			polish: "literary",
			voiceNotes: "Warm, unhurried, Southern-leaning American English. Short sentences mixed with longer remembering ones. Does not show off.",
			isSample: true,
			createdAt: now,
			updatedAt: now
		},
...
async function loadState() {
	try {
		...
		const value = await new Promise((resolve, reject) => {
			const req = db.transaction("kv", "readonly").objectStore("kv").get(STATE_KEY);
			req.onsuccess = () => resolve(req.result);
			req.onerror = () => reject(req.error);
		});
...
async function saveAudio(id, blob) {
	const tx = (await openDb()).transaction("audio", "readwrite");
...
var BookContext = (0, import_react.createContext)(null);
function BookProvider({ children }) {
	...
	const setCurrent = (0, import_react.useCallback)((bookId, chapterId) => {
		mutate((prev) => {
			const chapters = prev.chapters.filter((c) => c.bookId === bookId).sort((a, b) => a.sort - b.sort);
			const nextChapter = chapterId === void 0 ? chapters[0]?.id ?? null : chapterId;
			return {
				...prev,
	...
	const createBook = (0, import_react.useCallback)((input) => {
		const now = Date.now();
		const book = {
			id: uid("book"),
		...
		mutate((prev) => ({
			...prev,
			books: [
				...prev.books.filter((b) => !b.isSample),
				book,
				...prev.books.filter((b) => b.isSample)
			],
	...
	const updateBook = (0, import_react.useCallback)((id, patch) => {
		mutate((prev) => ({
			...prev,
	...
	const addChapter = (0, import_react.useCallback)((bookId, title) => {
	...
	const updateChapter = (0, import_react.useCallback)((id, patch) => {
		mutate((prev) => ({
			...prev,
			chapters: prev.chapters.map((c) => c.id === id ? {
				...c,
				...patch,
				updatedAt: Date.now()
			} : c)
	...
	const deleteChapter = (0, import_react.useCallback)((id) => {
		mutate((prev) => {
			const target = prev.chapters.find((c) => c.id === id);
			const remaining = prev.chapters.filter((c) => c.id !== id);
			const nextInBook = remaining.filter((c) => c.bookId === target?.bookId).sort((a, b) => a.sort - b.sort)[0];
			return {
				...prev,
				chapters: remaining,
				currentChapterId: prev.currentChapterId === id ? nextInBook?.id ?? null : prev.currentChapterId
			};
		});
	}, [mutate]);
	const addSession = (0, import_react.useCallback)(async (session, audio) => {
		if (audio) try {
	...
	const updateSettings = (0, import_react.useCallback)((patch) => {
		mutate((prev) => ({
			...prev,
			settings: {
				...prev.settings,
	...
	const chapters = (0, import_react.useMemo)(() => state.chapters.filter((c) => c.bookId === state.currentBookId).sort((a, b) => a.sort - b.sort), [state.chapters, state.currentBookId]);
	const chapter = chapters.find((c) => c.id === state.currentChapterId) ?? chapters[0] ?? null;
	const sessionsForChapter = (0, import_react.useMemo)(() => state.sessions.filter((s) => s.chapterId === chapter?.id).sort((a, b) => b.createdAt - a.createdAt), [state.sessions, chapter?.id]);
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BookContext.Provider, {
		value,
		children
	});
}
function useBook() {
	const ctx = (0, import_react.useContext)(BookContext);
	if (!ctx) throw new Error("useBook must be used within BookProvider");
	return ctx;
}
var styles_default = "/assets/styles-BWkqc-SN.css";
var APP_NAME = "Ghostwriter";
var Route$5 = createRootRoute({
	head: () => ({
		meta: [
			...
			{
				name: "description",
				content: "Speak your story. Ghostwriter sets it on the page in your voice, ready to read, hear, and print."
			}
		],
		links: [
			{
	...
	component: () => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("html", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("head", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(HeadContent, {}) }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("body", {
			className: "paper-grain min-h-dvh bg-paper text-ink",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(PreviewHostBridge, {}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(AuthProvider, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(BookProvider, { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Outlet, {}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Toaster, {
					position: "top-center",
					toastOptions: { className: "font-sans text-lg bg-paper text-ink border border-rule shadow-lg" }
				})] }) }),
...
var $$splitComponentImporter$1 = () => import("./routes-ZoDQ-DH2.mjs");
var Route$4 = createFileRoute("/")({ component: lazyRouteComponent($$splitComponentImporter$1, "component") });
var $$splitComponentImporter = () => import("./login-COTnCWZ2.mjs");
...
var Route$2 = createFileRoute("/api/transcribe")({ server: { handlers: { POST: async ({ request }) => {
	...
	if (!(file instanceof Blob) || file.size < 200) return Response.json({
		ok: false,
		error: "I did not receive a recording."
	}, { status: 400 });
	...
	try {
		...
		if (!res.ok) return Response.json({
			ok: false,
			error: `Could not hear the recording (${res.status}).`
		}, { status: 502 });
		let parsed = raw;
		try {
		...
		if (!text) return Response.json({
			ok: false,
			error: "I could not make out any words."
		}, { status: 422 });
...
function extractTranscript(parsed) {
	...
	if (Array.isArray(obj.segments)) {
		const joined = obj.segments.map((seg) => seg && typeof seg === "object" && "text" in seg ? String(seg.text ?? "") : "").join(" ").trim();
		if (joined) return joined;
	}
	if (obj.result && typeof obj.result === "object") return extractTranscript(obj.result);
	return "";
}
var Route$1 = createFileRoute("/api/tts")({ server: { handlers: { POST: async ({ request }) => {
	...
	if (!apiKey) return Response.json({
		ok: false,
		error: "Listening is not available right now."
	}, { status: 503 });
	...
	try {
		...
		if (!res.ok) return Response.json({
			ok: false,
			error: `Could not read the page (${res.status}).`
		}, { status: 502 });
...
//#endregion
export { getBaseURL as a, downloadText as c, uid as d, wordCount as f, PACKAGE_VERSION as i, formatClock as l, GROK_PROVIDERS as n, useBook as o, loadAudio as p, GENERIC_OAUTH_ERROR_CODES as r, cn as s, router_exports as t, formatLongDate as u };
